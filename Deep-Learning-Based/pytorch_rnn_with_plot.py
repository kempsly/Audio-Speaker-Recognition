...
def main():
    ...
    for epoch in range(num_steps):
        ...
        outputs = torch.squeeze(mynet(torch.from_numpy(inputs).float()))
        last_outputs = outputs[-1, :]
        loss = criterion(last_outputs, torch.from_numpy(train_labels).float())
        loss.backward()
        optimizer.step()

        train_losses.append(loss.item())

        # Evaluate on test data at each epoch
        with torch.no_grad():
